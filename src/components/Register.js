...
function Register({ setSnackbar, setIsLogin }) {
    ...
    const registerUser = async () => {
        ...
        if(errors.username) {
           return setSnackbar({ open: true, message: errors.username, variant: 'error' })
        } else if(errors.password) {
            return setSnackbar({ open: true, message: errors.password, variant: 'error' })
        } else if(errors.password2) {
            return setSnackbar({ open: true, message: errors.password2, variant: 'error' })
        } else {
            await axios.post(`${API_URL}/api/register`, inputs)
            setSnackbar({ open: true, message: 'נרשמת בהצלחה בבקשה להתחבר עם מייל וסיסמא', variant: 'success' })
            setIsLogin(true)
        }
    }
